...
class FixedSinusoidalEncoding(nn.Module):
    """固定正弦时频位置编码，替换动态编码，用于对比实验"""

    def __init__(self, dim, max_F=1000, max_T=1000):
        super().__init__()
        self.dim = dim
        self.max_F = max_F
        self.max_T = max_T
        ...
        # 预计算固定正弦位置编码
        self.register_buffer('pos_enc', self._get_sinusoidal_encoding(max_F, max_T, dim))

        # 融合模块（C + D → C）
        self.fusion_conv = nn.Conv2d(dim + dim, dim, kernel_size=1)

    def _get_sinusoidal_encoding(self, max_F, max_T, dim):
        """生成固定正弦位置编码，针对时频二维输入"""
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

        # 生成时间和频率的坐标网格
        f_coords = torch.arange(max_F, device=device).float()
        t_coords = torch.arange(max_T, device=device).float()
        grid = torch.stack(torch.meshgrid(f_coords, t_coords, indexing='ij'), dim=-1)  # [max_F, max_T, 2]

        # 计算正弦编码
        div_term = torch.exp(
            torch.arange(0, dim // 2, device=device).float() * (-math.log(10000.0) / (dim // 2)))  # [dim/2]
        pos_enc = torch.zeros(max_F, max_T, dim, device=device)

        # 频率维度编码
        pos_enc[..., 0::2] = torch.sin(grid[..., 0].unsqueeze(-1) * div_term)
        pos_enc[..., 1::2] = torch.cos(grid[..., 0].unsqueeze(-1) * div_term)

        return pos_enc.unsqueeze(0)  # [1, max_F, max_T, dim]
    ...
    def forward(self, x):
        ...
        # 获取适合输入尺寸的位置编码
        pos_enc = self.pos_enc[:, :F, :T, :].to(device)  # [1, F, T, dim]
        pos_enc = pos_enc.permute(0, 3, 1, 2).expand(B, -1, -1, -1)  # [B, dim, F, T]

        # 拼接输入和位置编码
        x_cat = torch.cat([x, pos_enc], dim=1)  # [B, C+dim, F, T]
...
class TimeFreqGlobalBlock(nn.Module):
    def __init__(self, dim, heads=8, reduction_ratio=4):
        ...
        # 位置编码
        self.dim = dim
        self.pos_enc = FixedSinusoidalEncoding(dim)
